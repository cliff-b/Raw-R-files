...
D5Map <- separate(D5Map, condition, into = c("template", "condition")) %>%
  separate("Y_peptide", into = c("Y_peptide", "Y_codon"), sep = "_") %>%
  separate ("X_peptide", into = c("X_peptide", "X_codon"), sep = "_rev_")
R20D5RNA <- filter(D5Map, grepl("RNA", D5Map$template)) %>%
  select(X_peptide, Y_peptide, condition, RevCompBC, "rcount" = nCount, count)
%>%
  separate(condition, into = c("template", "condition"))
sumRNA <- group_by(R20D5RNA, X_peptide, Y_peptide, condition) %>%
  summarise(mean_RNA = mean(rcount), sd_R = sd(rcount), bcnum = n_distinct(RevCompBC), tot_RNA = sum(count))
R20D5DNA <- filter(D5Map, grepl("DNA", D5Map$template)) %>%
  select(X_peptide, Y_peptide, condition, RevCompBC, "dcount" = nCount, count) 
%>%
  separate(condition, into = c("template", "condition"))
sumDNA <- group_by(R20D5DNA, X_peptide, Y_peptide, condition) %>%
  summarise(mean_DNA = mean(dcount), sd_R = sd(dcount), bcnum = n_distinct(RevCompBC), tot_RNA = sum(count))
R20D5RD <- inner_join(sumRNA,sumDNA, by = c("X_peptide","Y_peptide","condition")) %>%
  mutate("RNADNA" = mean_RNA/mean_DNA*100) 
%>%
  separate("Y_peptide", into = c("Y_peptide", "Y_codon"), sep = "_rev_") %>%
  separate ("X_peptide", into = c("X_peptide", "X_codon"), sep = "_")

CDvalues <- read.csv("161101 R20 CD values.csv")
colnames(CDvalues) <- c("X_peptide", "Y_peptide","nm222")
CDvalues$X_peptide <- paste(">", CDvalues$X_peptide, sep = "")
CDvalues$Y_peptide <- paste(">", CDvalues$Y_peptide, sep = "")
CDvalues$nm222 <- transmute(CDvalues, nm222*1000)
CDvalues <- left_join(CDvalues, sumR20D5, by = c("X_peptide", "Y_peptide"))

g.CDvsRD <- ggplot(filter(CDvalues, condition == "D"), aes(mean_RD, nm222)) + geom_point() + geom_smooth(method = lm, se = FALSE, color = "steelblue3") + 
  labs(title = "161101 mean R20 RNA/DNA counts vs CD MRE at 222nm", x = "Normalized mean RNA/DNA", y = "CD MRE at 222nm")
g.CDvsRD
...
g.recip <- ggplot(filter(sumR20D5, (condition == "D" | condition == "C") & X_peptide != ">GCN4" & Y_peptide != ">GCN4" & recip != 1.2), aes(med_RD, recip)) + geom_point() + 
  scale_x_log10(limits = c(10,1000)) + scale_y_log10(limits = c(10,1000)) + geom_smooth(method = glm, formula = y~x, color = "steelblue3")
g.recip

sumR20D5 <- mutate(sumR20D5, "double_mean" = (med_RD+recip)/2)
...
sumR20D5 <- group_by(R20D5RD, X_peptide, Y_peptide, condition) %>%
  summarise(mean_RNA = mean(rcount), sd_R = sd(rcount), mean_DNA = mean(dcount), sd_D = sd(dcount), mean_RD = mean(RNADNA), sd_RD = sd(RNADNA), med_RD = median(RNADNA), bcnum = n_distinct(RevCompBC), SEM = sd(RNADNA)/sqrt(n()))
...
g.sumR20RD <- ggplot(R20D5RD, aes(X_peptide, Y_peptide)) + geom_tile(aes(fill=RNADNA)) + facet_wrap(~condition, labeller = labeller(condition=induction_labels2), scales = "free") +
  ...
  scale_fill_viridis(option="viridis", name = "Ratio of \nRNA to DNA ", na.value="white") + labs(title="170223 Roman's 20 RNA/DNA collapsed by barcode", x="X peptide", y = "Y peptide")
g.sumR20RD

g.sumR20RDmed <- ggplot(filter(sumR20D5, !is.na(X_peptide) & condition != "A"), aes(X_peptide, Y_peptide)) + geom_tile(aes(fill=med_RD)) + facet_wrap(~condition, labeller = labeller(condition=induction_labels2), scales = "free", ncol = 5) +
  theme(text=element_text(family="Calibri"), strip.text=element_text(size = 10), strip.background=element_rect(fill = "White"), axis.text.x = element_text(angle=90, hjust=1,size =6),
        axis.text.y = element_text(size = 6), legend.title=element_text(size = 8), legend.title.align=0.5, legend.text=element_text(size =8)) +
