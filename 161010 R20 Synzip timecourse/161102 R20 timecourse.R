...
g.sumR20RD <- ggplot(filter(sumR20D5, !is.na(X_peptide)), aes(X_peptide, Y_peptide)) + geom_tile(aes(fill=SEM)) + facet_wrap(~condition, labeller = labeller(condition=induction_labels2), scales = "free") +
  ...
  scale_fill_viridis(option="viridis", name = "SEM of \nNormalized \nRNA to DNA ", na.value="white", limits = c(0,446)) + labs(title="161010 Roman's 20 RNA/DNA barcode ratio SEM", x="X peptide", y = "Y peptide")
g.sumR20RD
