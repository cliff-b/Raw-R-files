...
load("R20bigDF1.Rda")
load("GFPBCs.Rda")
bigDF <- select(bigDF, -barcode)
colnames(bigDF) <- c("count", "condition", "readsinlib", "barcode", "nCount")
GFPBCs <- select(GFPBCs, plasmid, "barcode" = RevCompBC)
mapGFP <- filter(left_join(bigDF,GFPBCs, by = "barcode"),!is.na(plasmid)) %>%
...
GFPRNA <- filter(mapGFP, grepl("RNA", mapGFP$condition)) %>%
  select(plasmid, clone, condition, barcode, "rcount" = nCount1 ) %>%
  separate(condition, into = c("template", "condition"))
GFPDNA <- filter(mapGFP, grepl("DNA", mapGFP$condition)) %>%
  select(plasmid, clone, condition, barcode, "dcount" = nCount1 ) %>%
  separate(condition, into = c("template", "condition"))
...
g.sumGFP <- ggplot(sumGFP, aes(plasmid, mean_RNADNA, fill= plasmid))  +geom_bar(stat="identity")+ facet_wrap(~condition, labeller = labeller(condition=induction_labels), scales = "free") + geom_errorbar(gfpERR) +  #geom_point(mapping=aes(plasmid,RNAoDNA), data=GFPRD) +
  labs(title="170215 R100 GFP constructs Average RNA/DNA counts", x ="plasmid", y = "Mean RNA/DNA") + scale_y_continuous(limits=c(0,700)) + theme(text = element_text(family="Calibri"), strip.background = element_rect(fill="white"))
g.sumGFP
...
D5Map <- filter(left_join(DNA5DF, mapBCs, by = "barcode"), !is.na(reads))
...
R20D5DNA <- filter(D5Map, grepl("DNA", D5Map$condition)) %>%
  ...
  separate(condition, into = c("template", "condition"))
R20D5RD <- left_join(R20D5RNA,R20D5DNA, by = c("barcode","X_peptide","Y_peptide","condition")) %>%
...
R20D5RD$dcount[is.na(R20D5RD$dcount)] <- 0
R20D5RD$RNADNA[is.na(R20D5RD$RNADNA)] <- 0
...
sumR20D52$condition[sumR20D52$condition == "A"] <- "D"
bigsum <- inner_join(sumR20D5,sumR20D52, by = c("X_peptide", "Y_peptide", "condition"))
bigsum <- mutate(bigsum, "med_RD.y" = med_RD.y*1.5)

g.bigs <- ggplot(filter(bigsum, condition == "D"), aes(log(mean_RD.x), log(mean_RD.y))) + geom_point() + geom_smooth( method = "lm", se = FALSE) + labs(x = "Biological rep 1", y = "Biological rep 2", title = "170320 Near Biological Replication of R20")
g.bigs
...
sumR20D52 <- group_by(R20D5RD, X_peptide, Y_peptide, condition) %>%
...
g.sumR20RD <- ggplot(filter(sumR20D52, !is.na(X_peptide)), aes(X_peptide, Y_peptide)) + geom_tile(aes(fill=med_RD)) + facet_wrap(~condition, labeller = labeller(condition=induction_labels2), scales = "free") +
  ...
  scale_fill_viridis(option="viridis", name = "SEM of \nNormalized \nRNA to DNA ", na.value="white", limits = c(0,356)) + labs(title="161010 Roman's 20 RNA/DNA barcode ratio SEM", x="X peptide", y = "Y peptide")
g.sumR20RD
...
g.sumR20RDbc <- ggplot(filter(sumR20D5, !is.na(X_peptide)), aes(X_peptide, Y_peptide)) + geom_tile(aes(fill=bcnum)) + facet_wrap(~condition, labeller = labeller(condition=induction_labels2), scales = "free") +
  theme(text=element_text(family="Calibri"), strip.text=element_text(size = 10), strip.background=element_rect(fill = "White"), axis.text.x = element_text(angle=90, hjust=1,size =6),
        axis.text.y = element_text(size = 6), legend.title=element_text(size = 8), legend.title.align=0.5, legend.text=element_text(size =8)) +
  scale_fill_viridis(option="viridis", name = "Number of\n barcodes ", na.value="white") + labs(title="170320 Roman's 20 number of barcodes per construct", x="X peptide", y = "Y peptide")
g.sumR20RDbc
