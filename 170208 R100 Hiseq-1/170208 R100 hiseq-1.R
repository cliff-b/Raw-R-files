...
setwd("/Users/Cliff/Documents/Kosuri Lab/NGS files/170208 R100 Hiseq-1/")
...
R100_RNA_8 <- read.table("counts_cDNA_8h.txt", header = FALSE)
R100_RNA_8 <- data.frame(R100_RNA_8, rep("RNA-8",nrow(R100_RNA_8)),rep(sum(R100_RNA_8$V1), nrow(R100_RNA_8)))
colnames(R100_RNA_8) <- c("count", "barcode", "condition","readsinlib")

R100_DNA_8 <- read.table("counts_pDNA_8h.txt", header = FALSE)
R100_DNA_8 <- data.frame(R100_DNA_8, rep("DNA-8",nrow(R100_DNA_8)),rep(sum(R100_DNA_8$V1), nrow(R100_DNA_8)))
colnames(R100_DNA_8) <- c("count", "barcode","condition","readsinlib")
...
R100_full_DF <- bind_rows(R100_RNA_0, R100_DNA_0, R100_RNA_4, R100_DNA_4, R100_RNA_8, R100_DNA_8)
remove(R100_RNA_0, R100_DNA_0, R100_RNA_4, R100_DNA_4, R100_RNA_8, R100_DNA_8)

load("R100_full_DF.Rda")
load("GFPBCs.Rda")
...
mapGFP <- filter(left_join(R100_full_DF,GFPBCs, by = "barcode"),!is.na(plasmid)) %>%
  ...
  mutate(mapGFP, "nCount" = count/readsinlib*1000000)
...
g.sumGFP <- ggplot(sumGFP, aes(plasmid, mean_RNADNA, fill= plasmid))  +geom_bar(stat="identity")+ facet_wrap(~condition, labeller = labeller(condition=induction_labels), scales = "free") + geom_errorbar(gfpERR) +  geom_point(mapping=aes(plasmid,RNAoDNA), data=GFPRD) +
  labs(title="170215 R100 GFP constructs Average RNA/DNA counts", x ="plasmid", y = "Mean RNA/DNA") + scale_y_continuous(limits=c(0,1400)) + theme(text = element_text(family="Calibri"), strip.background = element_rect(fill="white"))
g.sumGFP
...
R100_full_DF <- filter(R100_full_DF, X_peptide != "<NA>", Y_peptide != "<NA>")

R100RNA <- filter(R100_full_DF, grepl("RNA", R100_full_DF$condition)) %>%
  select(X_peptide, Y_peptide, condition, barcode, count, readsinlib, "rcount" = nCount ) %>%
  separate(condition, into = c("template", "condition"))
sumRNA <- group_by(R100RNA, X_peptide, Y_peptide, condition) %>%
  summarise(mean_RNA = mean(rcount), sd_R = sd(rcount), bcnum = n_distinct(barcode), tot_RNA = sum(count))
  
R100DNA <- filter(R100_full_DF, grepl("DNA", R100_full_DF$condition)) %>%
  select(X_peptide, Y_peptide, condition, barcode, count, "dcount" = nCount ) %>%
  separate(condition, into = c("template", "condition"))
sumDNA <- group_by(R100DNA, X_peptide, Y_peptide, condition) %>%
  summarise(mean_DNA = mean(dcount), sd_D = sd(dcount), bcnum_D = n_distinct(barcode), tot_DNA = sum(count))
R100RD <- right_join(R100RNA,R100DNA, by = c("X_peptide","Y_peptide","barcode","condition")) %>%
  mutate("RNADNA" = rcount/dcount*100) %>%
  select(-template.x, -template.y, -count.x, -count.y, -readsinlib) %>%
...
g.sumR100RD <- ggplot(sR100RD, aes(X_peptide, Y_peptide)) + geom_tile(aes(fill=RNADNA)) + facet_wrap(~condition, labeller = labeller(condition=induction_labels), scales = "free") +
  ...
  scale_fill_viridis(option="viridis", name = "Mean \nRNA  ", na.value="white") + labs(title="170214 Roman's 100 RNA/DNA barcode ratio", x="X peptide", y = "Y peptide")
g.sumR100RD
