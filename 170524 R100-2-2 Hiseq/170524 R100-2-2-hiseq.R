...
R100_raw_map1 <- read.table("sc-counts_R100-c1_R2.txt", header = FALSE)
colnames(R100_raw_map1) <- c("barcode", "count", "collapsedBC")
R100_raw_map2 <- read.table("sc-counts_R100-c2_R2.txt", header = FALSE)
colnames(R100_raw_map2) <- c("barcode", "count", "collapsedBC")
R100_raw_map3 <- read.table("sc-counts_R100-c3_R2.txt", header = FALSE)
colnames(R100_raw_map3) <- c("barcode", "count", "collapsedBC")
R100_raw_map <- bind_rows(R100_raw_map1, R100_raw_map2, R100_raw_map3)
R100_raw_map <- select(R100_raw_map, -collapsedBC)
R100_raw_map <- distinct(R100_raw_map, barcode, count)


load("/Users/Cliff/Documents/Kosuri Lab/NGS files/170509 R100-2 mapping/R100bcs.Rda")
R100map <- filter(R100bcs, X_peptide != "<NA>", Y_peptide != "<NA>")
...
otherbcs <- R100map[(duplicated(R100map$barcodes) | duplicated(R100map$barcodes, fromLast = TRUE)),]

R100_full_DF <- mutate(R100_full_DF, "nCount" = count/readsinlib*1000000)
R100_full_DF <- inner_join(R100_full_DF, R100map, by = c("barcode" = "barcodes"))
