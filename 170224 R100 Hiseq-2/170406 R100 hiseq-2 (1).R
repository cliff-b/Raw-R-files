...
R100_full_DF3 <- left_join(R100_full_DF, mapBCs, by = "barcode")
...
%>%
  separate(condition, into = c("template", "condition"))
g.RNA <- ggplot(R100RNA, aes(rcount0, rcount4)) + geom_density2d(alpha = 0.5) + labs(x = "Normalized RNA counts at 0 hours", y = "Normalized RNA counts at 4 hours", title = "170406 R100 normalized RNA over time") + geom_abline(slope = 1)+
  scale_x_continuous(limits = c(0,1200)) + scale_y_continuous(limits = c(0, 1200))
  #scale_x_log10(limits = c(0.018, 1500)) + scale_y_log10(limits = c(0.018, 1500))
g.RNA


R100DNA <- filter(R100_4, grepl("DNA", R100_4$condition)) %>%
  select(X_peptide, Y_peptide, condition, barcode, "dcountx" = nCount.x, "dcounty" = nCount.y ) %>%
  separate(condition, into = c("template", "condition"))
g.DNA <- ggplot(R100DNA, aes(dcountx, dcounty, color=condition)) + geom_density2d(alpha = 0.2) + labs(x = "technical replicate 2", y = "technical replicate 1", title = "170404 R100 normalized DNA replicates") + geom_abline(slope=1)+
  scale_x_continuous(limits limits = c(0,1200))
  #scale_x_log10(limits = c(0.028, 700)) + scale_y_log10(limits = c(0.028, 700))
g.DNA


R100RD <- right_join(R100RNA,R100DNA, by = c("X_peptide","Y_peptide","barcode","condition")) %>%
  mutate("RNADNAx" = rcountx/dcountx*100) %>%
  mutate("RNADNAy" = rcounty/dcounty*100) %>%
  select(-template.x, -template.y) %>%
  mutate("RNADNAx" = ifelse(is.na(RNADNAx),0,RNADNAx)) %>%
